...
const s = {
  page: { minHeight: '100vh', background: '#0a0f1e', display: 'flex',  position: 'relative', alignItems: 'center', justifyContent: 'center', padding: 24 },
  ...
  wordmark: { fontFamily: 'var(--font-display)', fontSize: 32, color: 'var(--gold)' },
...
export default function OperatorLogin() {
  ...
  return (
    <div style={s.page}>
      <div style={s.box}>
        <div style={s.logo}>
          <div style={s.wordmark}><span style={{ color: 'var(--gold)' }}>Sky</span>Vayu</div>
      ...
      {showForgot && (
        <div onClick={() => setShowForgot(false)} style={{ position: 'fixed', inset: 0,background: 'rgba(0,0,0,0.75)',display: 'flex',justifyContent: 'center',alignItems: 'center',zIndex: 99999,}} onClick={() => setShowForgot(false)}>
          <div style={{ position: 'relative', width: '400px', background: '#111827', border: '1px solid rgba(255,255,255,0.12)', borderRadius: 8,padding: '30px', }} onClick={e => e.stopPropagation()}>
             <button
        onClick={() => setShowForgot(false)} style={{ position: 'absolute', top: 12, right: 15, background: 'transparent', border: 'none',color: '#fff',fontSize: 22,cursor: 'pointer',
        }}>
        ✕
      </button>
            ...
            {forgotSent ? (
              <div style={{ color: '#4caf50',  textAlign: 'center', padding: '20 px 0' }}>✓ Reset link sent! Check your email.</div>
            ) : (
              <>
                ...
                <button style={s.btn} onClick={async () => {
                  try { await import('../../services/api').then(m => m.authApi.forgotPassword(forgotEmail)); setForgotSent(true) } catch {}
...
function RegisterModal({ onClose }) {
  ...
  const s2 = {
    overlay: { position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.7)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000, overflowY: 'auto', padding: 24 },
        ...
        {success ? (
          ...
          <>
            ...
            {[
              ...
              ['Address', 'address', 'address', 'Fill the address'],
              ...
              <div key={key}>
                ...
                <input style={s2.inp} type={type} placeholder={placeholder} value={form[key]} onChange={(e) => {update(key)(e);
